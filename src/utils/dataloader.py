...
class TrainDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, shuffle=False):
        ...
        if self.neighborhood_loss_required:
            self.history_users_per_i = self._get_history_users_i()
    ...
    def _get_neg_sample(self):
        ...
        neg_ids = self._sample_neg_ids(u_ids).to(self.device) # sampling negative items only in the dataset (train)
    ...
    def _get_history_items_u(self):
        ...
        for u, u_ls in uid_freq:
            history_items_per_u[u] = set(u_ls.values)
        return history_items_per_u
    ...
    def _get_history_users_i(self):
        ...
        for i, u_ls in iid_freq:
            history_users_per_i[i] = set(u_ls.values)
        return history_users_per_i
